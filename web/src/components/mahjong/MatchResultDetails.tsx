...
export default function MatchResultDetails({ details }: MatchResultDetailsProps) {
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      <div className="bg-background border border-foreground/10 rounded-2xl p-6 shadow-lg">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-black text-blue-600 dark:text-blue-400">대국 종료</h2>
          <p className="text-sm font-bold opacity-60 mt-1">기록 점수 기반 순위 정산</p>
        </div>
            ...
            return (
              <div
                ...
                className={`p-4 rounded-xl border transition-all ${
                  isFirst ? "bg-yellow-50 border-yellow-200 dark:bg-yellow-900/20" : "bg-foreground/5 border-foreground/5"
                }`}
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center font-black text-sm ${
                      isFirst ? "bg-yellow-400 text-white" : "bg-foreground/10"
                    }`}>
                      {index + 1}
                    </div>
                ...
                <div className="flex justify-end gap-4 text-xs font-bold opacity-60">
                  <span className={umaValue >= 0 ? "text-blue-600" : "text-red-600"}>
                    {umaValue > 0 ? "+" : ""}{umaValue}
                  </span>
                </div>
              </div>
